...
public class ContainerItem extends Container {
	...
	public InventoryItemBag inventory;

	/**
	 * Using these will make transferStackInSlot easier to understand and implement INV_START is the index of the first slot in the Player's Inventory, so our InventoryItem's
	 * number of slots (e.g. 5 slots is array indices 0-4, so start at 5) Notice how we don't have to remember how many slots we made? We can just use InventoryItem.INV_SIZE
	...
	public ContainerItem(EntityPlayer par1Player, InventoryPlayer inventoryPlayer, InventoryItemBag inventoryItem) {
	...
	/**
	 * Called when a player shift-clicks on a slot. You must override this or you will crash when someone does that. Only real change we make to this is to set needsUpdate to
	 * true at the end
	 */
	@Override
